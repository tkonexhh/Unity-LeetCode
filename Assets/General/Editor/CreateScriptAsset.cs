...
{
    [MenuItem("Assets/Create/Script/CreateLeetCodeScripts", priority = 0)]
    private static void CreatePanelScript()
    {
        ProjectWindowUtil.StartNameEditingIfProjectWindowExists(0,
                    ScriptableObject.CreateInstance<CreateScriptAsset>(),
                    GetSelectPathOrFallback() + "/LeetCode_X.cs", null,
                    "Assets/General/ScriptModel.cs.txt");
    }
...
public class CreateScriptAsset : EndNameEditAction
{
    ...
    internal static UnityEngine.Object CreateScriptAssetFromTemplate(string pathName, string resourceFile)
    {
        ...
        fileNameWithoutExtension = fileNameWithoutExtension.Substring(9);
        Debug.LogError(fileNameWithoutExtension);
        //将模板类中的类名替换成你创建的文件名
        text = Regex.Replace(text, "#ID#", fileNameWithoutExtension);
